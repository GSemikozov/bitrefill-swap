...
**Spam filtering** (`filterDisplayableTokens`): any active address accumulates unsolicited spam
airdrops, and `getTokenBalances` returns them all. They share one tell — no USD price (no
market). The rule: native ETH always shows; a priced token shows unless it's dust (< $0.01); an
*unpriced* token shows only if it's on the curated whitelist. That filters the junk while
keeping the no-Alchemy fallback (curated, unpriced) working — and an unpriced non-curated token
has no Uniswap route anyway, so it couldn't be swapped to USDC even if shown. The pure function
is unit-tested; the earlier "never hide what we can't value" stance was reversed once a live
wallet surfaced the spam.

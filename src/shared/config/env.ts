...
/**
 * Demo mode is runtime-switchable from the UI: a localStorage override wins,
 * the env var only sets the default. Call-time (not module-level) so tests
 * can toggle it.
 */
...
/** The caller is expected to reload — the mode affects non-reactive modules. */

...
- This is exposed as **demo mode**: header toggle backed by a reactive Zustand store
  (`shared/config/demo.ts`) — flips in place, NO page reload (which would remount wallet
  providers and flicker); resets the flow on switch; only switchable in idle/selecting.
  `isDemoPayment()` = call-time read for non-reactive modules, `useIsDemoPayment()` = reactive
  hook for components; localStorage persists, `VITE_DEMO_PAYMENT=balance` only sets the default.
  Token-list query key includes demo so flipping re-derives the curated fallback.
